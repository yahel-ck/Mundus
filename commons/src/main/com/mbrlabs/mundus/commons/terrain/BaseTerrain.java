 ...
 * @version 30-11-2015
 */
public abstract class BaseTerrain implements TerrainInfo, Disposable {
    ...
    protected BaseTerrain(int vertexResolution, int terrainWidth, int terrainDepth) {
        this.attribs = new VertexAttributes(VertexAttribute.Position(), VertexAttribute.Normal(),
    ...
    /**
     * This method builds a map that associates each vertex index with a list of indices of triangles that the vertex is
     * part of. This map is used for efficient lookup of adjacent triangles when calculating vertex normals.
     *
     * The map is stored in the instance variable vertexToTriangleMap, where the key is the vertex index and the value
     * is a list of triangle indices.
     *
     * Note: This method is to be called during the mesh building process, after the indices array has been populated.
     */
    ...
    /**
     * This method calculates and sets the average normal for each vertex in the terrain mesh. It first calculates the
     * normal of each face (triangle) in the mesh, then for each vertex, it calculates the average normal from the
     * normals of all faces that include this vertex.
     *
     * @param numIndices
     *         The total number of indices in the index buffer, representing the total number of vertices in the mesh.
     * @param numVertices
     *         The total number of unique vertices in the vertex buffer.
     *
    ...
    /**
     * This method calculates the normal of a face in 3D space given its three vertices. The face is assumed to be a
     * triangle.
     *
     * @param out
    ...
    /**
     * This method calculates the average normal of a vertex by averaging the normals of all the faces that the vertex
     * is part of.
     *
     * @param vertexIndex
     *         The index of the vertex for which the normal is to be calculated.
     * @param faceNormals
     *         An array containing the normals of all faces in the mesh.
    ...
    public Vector3 getVertexPosition(Vector3 out, int x, int z) {
        ...
        final float height = getHeight(z * vertexResolution + x);
     ...
     *
     * @param worldX
     *         X world position to get height
     * @param worldZ
     *         Z world position to get height
    ...
    public float getHeightAtWorldCoord(float worldX, float worldZ, Matrix4 terrainTransform) {
        ...
        c01.set(1, getHeight((gridZ + 1) * vertexResolution + gridX), 0);
        c10.set(0, getHeight(gridZ * vertexResolution + gridX + 1), 1);

        float height;
        if (xCoord <= (1 - zCoord)) { // we are in upper left triangle of the square
            c00.set(0, getHeight(gridZ * vertexResolution + gridX), 0);
            height = MathUtils.barryCentric(c00, c10, c01, tmpV2.set(zCoord, xCoord));
        } else { // bottom right triangle
            c11.set(1, getHeight((gridZ + 1) * vertexResolution + gridX + 1), 1);
     ...
     *
     * @param out
     *         Vector3 to populate with intersect point with
     * @param ray
     *         the ray to cast
     * @param terrainTransform
     *         The world transform (modelInstance transform) of the terrain
    ...
    public boolean isUnderTerrain(Vector3 worldCoords, Matrix4 terrainTransform) {
        ...
        float terrainHeight = getHeightAtWorldCoord(worldCoords.x, worldCoords.z,
                terrainTransform) + terrainTransform.getTranslation(tmp).y;
